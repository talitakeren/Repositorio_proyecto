/** Catálogo de restricciones CSP (solo lectura para el panel admin). */
export const RESTRICTIONS_CATALOG = [
  ...
  },
  {
    ...
    examples:
      "STANDARD → Aula estándar · LAB → Laboratorio · COMPUTER_ROOM → Sala de cómputo",
  },
  {
  ...
  },
  {
    ...
    validation: "Enrollment.status ∈ { CONFIRMED, VALIDATED, VALID }",
    impact:
...
export function buildRestrictionsSummary(restrictions = RESTRICTIONS_CATALOG) {
  const active = restrictions.filter((r) => r.status === "ACTIVE");
  return {
    totalActive: active.length,
    hardCount: active.filter((r) => r.type === "HARD").length,
    operationalCount: active.filter((r) => r.type === "OPERATIONAL").length,
    restrictions: active,
    hard: active.filter((r) => r.type === "HARD"),
    operational: active.filter((r) => r.type === "OPERATIONAL"),
  };
}
