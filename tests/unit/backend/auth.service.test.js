...
describe("authService", () => {
  ...
  // --- spy: verifica que login consulta la BD con el email normalizado ---
  test("login llama a User.findOne con email en minúsculas (spy)", async () => {
    await seedAdmin();
    ...
    expect(spy).toHaveBeenCalledWith(
      expect.objectContaining({ email: "admin-test@sgoha.edu" })
    );
    spy.mockRestore();
  });
});