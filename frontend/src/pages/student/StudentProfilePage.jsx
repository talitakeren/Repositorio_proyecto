...
export default function StudentProfilePage() {
  ...
  if (linkError || !student) {
    return (
      <div className="space-y-6">
        <PageHeader title="Mi perfil académico" />
        <Card className="flex items-start gap-3 border-amber-200 bg-amber-50 p-5 text-amber-900">
          <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0" />
          <p className="text-sm">
            Tu usuario aún no tiene un perfil de alumno vinculado.
          </p>
        </Card>
      </div>
    );
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col items-center gap-4 border-b border-slate-100 bg-gradient-to-br from-[#1E3A8A] to-[#172554] p-6 text-center text-white sm:flex-row sm:text-left">
          ...
          <div>
            ...
            <p className="text-sm text-blue-100/90">{student.code}</p>
        ...
        <dl className="grid grid-cols-1 gap-px bg-slate-100 sm:grid-cols-2">
          <Info icon={Hash} label="Código" value={student.code} />
          <Info icon={Mail} label="Correo institucional" value={student.email} />
          <Info
            icon={GraduationCap}
          ...
          <Info
            ...
            value={`${student.approvedCourses?.length || 0} cursos`}
          />
        </dl>
      </Card>
